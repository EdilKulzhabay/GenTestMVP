  ...
  paths: {
    ...
    '/roadmaps/nodes/{nodeId}/lesson': {
      get: {
        ...
        description:
          'Узел = тема КТП и может содержать несколько уроков (`metadata.lessons[]`). Без `lessonId` возвращается первый незавершённый урок. Ответ включает список уроков с флагами completed/locked и навигацию (nextLessonId/prevLessonId). Базовый путь API: `/api/v1/roadmaps/...`.',
        security: [{ cookieAuth: [] }, { bearerAuth: [] }],
        parameters: [
