...
Привязка гостевого результата к пользователю: \`POST …/tests/claim-guest\`.`
    ...
    { name: 'KTP', description: 'Справочник КТП (календарно-тематическое планирование) по предмету: эталон тем (CRUD — admin) и маппинг тем книг на КТП (teacher+admin). Роудмап строится из КТП.' },
    {
      name: 'Users',
...
Обрыв связи у **хоста** в лобби не закрывает комнату сразу: действует grace-окно (по умолчанию 45 с, env \`LIVE_HOST_GRACE_MS\`), в течение которого \`live:rejoin\` возвращает хоста с актуальным \`state\`. Только по истечении окна комната закрывается (\`live:room_closed\`, \`reason: 'host_left'\`). Явный \`live:lobby_leave\` хоста закрывает комнату немедленно.
...
После создания сессии через REST используйте последовательность \`solo:join\` → \`solo:answer\` → \`solo:finish\`. Схемы: **SocketSolo*** в **Components**.`
  ...
  components: {
    ...
    schemas: {
      ...
      GenerateTestRequest: {
        ...
        properties: {
          ...
          forTrial: {
            ...
            default: false,
            description:
              'Пробник: при `true` сервер не подменяет ответ «последним сохранённым тестом по предмету», а генерирует тест по текущим `bookId`/`chapterId`/`testProfile`. Нужно, чтобы в `submit` корректно посчитался `trialTopicMastery` и POST /trial/apply-results открыл узлы roadmap.'
          }
        },
        example: {
      ...
      EntProgress: {
        ...
        properties: {
          ...
          data: {
            type: 'object',
            properties: {
              score: { type: 'number', description: 'Прогноз в шкале ЕНТ (0..max)', example: 74 },
              max: { type: 'number', example: 140 },
              updatedAt: { type: 'string', format: 'date-time', description: 'Дата последнего результата, повлиявшего на прогноз' },
              hasProfilePair: { type: 'boolean' },
              subjects: {
                type: 'array',
                description: 'Разбивка по блокам ЕНТ: 3 обязательных + до 2 профильных из пары',
                items: {
                  type: 'object',
                  properties: {
                    subjectId: { type: 'string', nullable: true },
                    title: { type: 'string', example: 'История Казахстана' },
                    kind: { type: 'string', enum: ['main', 'profile'] },
                    maxPoints: { type: 'number', example: 20 },
                    predictedPoints: { type: 'number', example: 14 },
                    accuracyPercent: { type: 'number', nullable: true, description: 'null — нет отвеченных вопросов по предмету' },
          ...
          data: {
            ...
            properties: {
              achievements: {
                type: 'array',
                description:
                  'Статусы по id из статичного каталога клиента. Достижения без серверных событий (bookworm, fast, rising) не включаются; half-way/full-set — только когда у пользователя есть темы роадмапа (topics.total > 0).',
                items: {
                  type: 'object',
                  properties: {
            ...
            properties: {
              ...
              breakdown: {
                ...
                properties: {
                  ...
                  solo: { type: 'number', description: 'Speed-очки ranked solo-кахутов ÷ 10 (practice не начисляется)' },
                  live: { type: 'number', description: 'Speed-очки live-матчей с соперниками (≥2 участников) ÷ 10' }
                }
              },
              updatedAt: { type: 'string', format: 'date-time' }
            }
      ...
      KtpTopic: {
        type: 'object',
        description: 'Тема КТП. _id — стабильный id (на него ссылаются Subject.Topic.ktpTopicIds и nodeId = ktp:{_id})',
        properties: {
          _id: { type: 'string' },
          title: { type: 'string' },
          description: { type: 'string' },
          order: { type: 'integer' },
          code: { type: 'string', description: 'Код темы от центра тестирования (отображение/импорт)' }
        }
      },
      KtpCatalogResponse: {
        type: 'object',
        nullable: true,
        description: 'Справочник КТП по предмету (null — ещё не заведён)',
        properties: {
          _id: { type: 'string' },
          subjectId: { type: 'string' },
          year: { type: 'integer', example: 2026 },
          version: { type: 'integer', example: 1 },
          topics: { type: 'array', items: { $ref: '#/components/schemas/KtpTopic' } }
        }
      },
      PersonalRoadmapNode: {
        type: 'object',
      ...
      RoadmapLessonListItem: {
        type: 'object',
        description: 'Элемент списка уроков узла (степпер с последовательным гейтингом)',
        properties: {
          lessonId: { type: 'string' },
          title: { type: 'string' },
          order: { type: 'integer' },
          completed: { type: 'boolean' },
          locked: { type: 'boolean', description: 'Заблокирован: предыдущий по порядку урок не завершён' }
        }
      },
      RoadmapLessonResponse: {
        type: 'object',
        description: 'Узел = тема КТП; может содержать несколько уроков (по одному на замапленную тему книги из разных классов).',
        properties: {
          nodeId: { type: 'string' },
          lessonId: { type: 'string', description: 'id текущего урока внутри узла' },
          title: { type: 'string' },
          summary: { type: 'string', description: 'Краткая выжимка (Markdown), кэшируется после ИИ (только для не-live карт)' },
          ...
          readCompletedAt: { type: 'string', format: 'date-time', nullable: true, description: 'Время завершения ТЕКУЩЕГО урока' },
          lessons: { type: 'array', items: { $ref: '#/components/schemas/RoadmapLessonListItem' } },
          lessonsTotal: { type: 'integer' },
          lessonIndex: { type: 'integer', description: 'Позиция текущего урока (0-based)' },
          nextLessonId: { type: 'string', nullable: true },
          prevLessonId: { type: 'string', nullable: true },
          locked: { type: 'boolean', description: 'Текущий урок заблокирован (предыдущий не завершён)' },
          sources: {
            type: 'array',
            description: 'Источники узла (темы книг разных классов), из которых AI-консолидацией собран урок',
            items: { type: 'object', properties: { bookTitle: { type: 'string' }, topicTitle: { type: 'string' } } }
          }
        }
      },
      RoadmapLessonReadResponse: {
        ...
        properties: {
          ...
          lessonId: { type: 'string' },
          allCompleted: { type: 'boolean', description: 'Все уроки узла завершены (узел «материал пройден»)' },
          nextLessonId: { type: 'string', nullable: true }
        }
      },
  ...
  paths: {
    ...
    '/subjects/{id}': {
      ...
      delete: {
        ...
        description:
          'Безвозвратно удаляет предмет и связанные данные: тесты и соло-сессии по ним (`SoloSession`, `SoloAttempt`), `CanonicalRoadmap`, `UserRoadmapProgress`, попытки roadmap, вложения чата roadmap, пары профилей, где участвует предмет (у затронутых пользователей сбрасывается `profileSubjectPairId`), записи в `User.testHistory` по этому предмету.',
        security: [{ cookieAuth: [] }, { bearerAuth: [] }],
        parameters: [
          { name: 'id', in: 'path', required: true, description: 'Mongo ObjectId предмета', schema: { type: 'string' } }
        ],
        responses: {
          200: {
            description: 'Удалено',
            content: {
              'application/json': {
                schema: {
                  ...
                  properties: {
                    success: { type: 'boolean', example: true },
                    message: { type: 'string', example: 'Subject deleted' },
                    data: {
                      type: 'object',
          ...
          400: { description: 'Невалидный subject id' },
          401: { description: 'Не авторизован' },
    ...
    '/tests/generate': {
      post: {
        ...
        description:
          'Без `forTrial` сервер может вернуть «последний сохранённый тест по предмету» (быстрый путь для Live Kahoot и т.п.). С `forTrial: true` всегда генерирует под выбранную книгу/главу и сохраняет `relatedContent.topicId` в вопросах — это нужно для пробника, чтобы POST /trial/apply-results корректно открыл узлы roadmap.',
    ...
    '/tests/generate-guest': {
      post: {
        ...
        description:
          'Аналогично `/tests/generate` для гостя: с `forTrial: true` пропускается подстановка «последнего теста по предмету», и тест генерируется под текущие `bookId`/`chapterId` (нужно для пробника).',
    ...
    '/roadmaps/nodes/{nodeId}/lesson': {
      get: {
        ...
        description:
          'Узел = тема КТП = один консолидированный урок (или 2–4 при большом объёме). Контент собирается AI-консолидацией из текстов замапленных тем книг и кэшируется (NodeLessonContent); без OPENAI_API_KEY — сырой fallback. Без `lessonId` возвращается первый незавершённый урок. Ответ включает список уроков (completed/locked), навигацию (nextLessonId/prevLessonId) и `sources`. Базовый путь API: `/api/v1/roadmaps/...`.',
        security: [{ cookieAuth: [] }, { bearerAuth: [] }],
        parameters: [
          { name: 'nodeId', in: 'path', required: true, schema: { type: 'string' } },
          { name: 'subjectId', in: 'query', required: true, schema: { type: 'string' } },
          { name: 'lessonId', in: 'query', required: false, schema: { type: 'string' }, description: 'Конкретный урок узла; по умолчанию — первый незавершённый' }
        ],
        responses: {
          200: {
            description: 'Урок',
    ...
    '/roadmaps/nodes/{nodeId}/lesson/read': {
      post: {
        tags: ['Roadmaps'],
        summary: 'Отметить урок узла завершённым',
        description:
          'Последовательный гейтинг: нельзя завершить урок, пока не завершены предыдущие. Без `lessonId` завершается первый незавершённый урок. Завершение последнего урока ставит узлу «материал пройден».',
        security: [{ cookieAuth: [] }, { bearerAuth: [] }],
        parameters: [{ name: 'nodeId', in: 'path', required: true, schema: { type: 'string' } }],
        requestBody: {
          required: true,
          content: {
            'application/json': {
              schema: {
                ...
                properties: {
                  subjectId: { type: 'string' },
                  lessonId: { type: 'string', description: 'Урок узла; по умолчанию — первый незавершённый' }
                }
              }
            }
          }
    ...
    '/roadmaps/admin/rebuild-from-ktp': {
      post: {
        tags: ['Roadmaps'],
        summary: 'Актуализировать карту знаний из КТП (teacher/admin)',
        description:
          'Проверяет, что КТП заведён и хотя бы одна тема книги замаплена; удаляет устаревший сохранённый снимок и бампает updatedAt предмета (версия карты для учеников пересчитывается). Карта строится из КТП вживую. Алиас: POST /roadmaps/admin/rebuild-from-topics.',
        security: [{ cookieAuth: [] }, { bearerAuth: [] }],
        requestBody: {
          required: true,
        ...
        responses: {
          ...
          400: { description: 'Нет КТП или ни одна тема книги не замаплена' }
        }
      }
    },
    '/roadmaps/admin/rebuild-from-topics': {
      post: {
        tags: ['Roadmaps'],
        summary: 'Алиас POST /roadmaps/admin/rebuild-from-ktp (deprecated)',
        deprecated: true,
        description: 'Историческое имя. Поведение идентично /roadmaps/admin/rebuild-from-ktp (сборка из КТП).',
        security: [{ cookieAuth: [] }, { bearerAuth: [] }],
        requestBody: {
          required: true,
          content: { 'application/json': { schema: { $ref: '#/components/schemas/RebuildCanonicalFromTopicsRequest' } } }
        ...
        responses: {
          200: { description: 'subjectId, version, nodesCount' },
          400: { description: 'Нет КТП или ни одна тема книги не замаплена' }
        }
      }
    },

    // ==================== KTP (справочник КТП и маппинг) ====================
    '/ktp/{subjectId}': {
      get: {
        tags: ['KTP'],
        summary: 'Справочник КТП предмета (teacher/admin)',
        description: 'Темы КТП по порядку. null — справочник ещё не заведён. Чтение доступно учителю (нужно для маппинга).',
        security: [{ cookieAuth: [] }, { bearerAuth: [] }],
        parameters: [{ name: 'subjectId', in: 'path', required: true, schema: { type: 'string' } }],
        responses: {
          200: { description: 'Справочник или null', content: { 'application/json': { schema: { type: 'object', properties: { success: { type: 'boolean' }, data: { $ref: '#/components/schemas/KtpCatalogResponse' } } } } } },
          404: { description: 'Предмет не найден' }
        }
      },
      post: {
        tags: ['KTP'],
        summary: 'Параметры справочника КТП: год/версия (admin)',
        security: [{ cookieAuth: [] }, { bearerAuth: [] }],
        parameters: [{ name: 'subjectId', in: 'path', required: true, schema: { type: 'string' } }],
        requestBody: {
          content: { 'application/json': { schema: { type: 'object', properties: { year: { type: 'integer', example: 2026 }, version: { type: 'integer', example: 1 } } } } }
        },
        responses: { 201: { description: 'Справочник создан/обновлён', content: { 'application/json': { schema: { type: 'object', properties: { success: { type: 'boolean' }, data: { $ref: '#/components/schemas/KtpCatalogResponse' } } } } } } }
      }
    },
    '/ktp/{subjectId}/topics': {
      post: {
        tags: ['KTP'],
        summary: 'Добавить тему КТП (admin)',
        security: [{ cookieAuth: [] }, { bearerAuth: [] }],
        parameters: [{ name: 'subjectId', in: 'path', required: true, schema: { type: 'string' } }],
        requestBody: {
          required: true,
          content: { 'application/json': { schema: { type: 'object', required: ['title'], properties: { title: { type: 'string' }, description: { type: 'string' }, order: { type: 'integer' }, code: { type: 'string' } } } } }
        },
        responses: { 201: { description: 'Тема добавлена; возвращён весь справочник', content: { 'application/json': { schema: { type: 'object', properties: { success: { type: 'boolean' }, data: { $ref: '#/components/schemas/KtpCatalogResponse' } } } } } } }
      }
    },
    '/ktp/{subjectId}/topics/{topicId}': {
      patch: {
        tags: ['KTP'],
        summary: 'Изменить тему КТП (admin)',
        security: [{ cookieAuth: [] }, { bearerAuth: [] }],
        parameters: [
          { name: 'subjectId', in: 'path', required: true, schema: { type: 'string' } },
          { name: 'topicId', in: 'path', required: true, schema: { type: 'string' } }
        ...
        requestBody: {
          content: { 'application/json': { schema: { type: 'object', properties: { title: { type: 'string' }, description: { type: 'string' }, order: { type: 'integer' }, code: { type: 'string' } } } } }
        },
        responses: { 200: { description: 'Обновлено', content: { 'application/json': { schema: { type: 'object', properties: { success: { type: 'boolean' }, data: { $ref: '#/components/schemas/KtpCatalogResponse' } } } } } } }
      },
      delete: {
        tags: ['KTP'],
        summary: 'Удалить тему КТП (admin)',
        security: [{ cookieAuth: [] }, { bearerAuth: [] }],
        parameters: [
          { name: 'subjectId', in: 'path', required: true, schema: { type: 'string' } },
          { name: 'topicId', in: 'path', required: true, schema: { type: 'string' } }
        ],
        responses: { 200: { description: 'Удалено; возвращён весь справочник' } }
      }
    },
    '/ktp/{subjectId}/reorder': {
      post: {
        tags: ['KTP'],
        summary: 'Переупорядочить темы КТП (admin)',
        security: [{ cookieAuth: [] }, { bearerAuth: [] }],
        parameters: [{ name: 'subjectId', in: 'path', required: true, schema: { type: 'string' } }],
        requestBody: {
          required: true,
          content: { 'application/json': { schema: { type: 'object', required: ['orderedTopicIds'], properties: { orderedTopicIds: { type: 'array', items: { type: 'string' }, description: 'Полный список id тем в нужном порядке' } } } } }
        },
        responses: { 200: { description: 'Порядок обновлён' } }
      }
    },
    '/ktp/{subjectId}/import': {
      post: {
        tags: ['KTP'],
        summary: 'Массовый импорт тем КТП из JSON (admin)',
        description: 'replace=true (по умолчанию) заменяет темы; replace=false — добавляет в конец.',
        security: [{ cookieAuth: [] }, { bearerAuth: [] }],
        parameters: [{ name: 'subjectId', in: 'path', required: true, schema: { type: 'string' } }],
        requestBody: {
          required: true,
          content: { 'application/json': { schema: { type: 'object', required: ['topics'], properties: { topics: { type: 'array', items: { type: 'object', required: ['title'], properties: { title: { type: 'string' }, description: { type: 'string' }, order: { type: 'integer' }, code: { type: 'string' } } } }, year: { type: 'integer' }, version: { type: 'integer' }, replace: { type: 'boolean' } } } } }
        },
        responses: { 201: { description: 'Импортировано; возвращён весь справочник', content: { 'application/json': { schema: { type: 'object', properties: { success: { type: 'boolean' }, data: { $ref: '#/components/schemas/KtpCatalogResponse' } } } } } } }
      }
    },
    '/subjects/{subjectId}/books/{bookId}/chapters/{chapterId}/topics/{topicId}/ktp': {
      put: {
        tags: ['KTP'],
        summary: 'Маппинг темы книги на темы КТП (teacher/admin)',
        description: 'M:N: одна тема книги может относиться к нескольким темам КТП. ktpTopicIds валидируются по справочнику КТП предмета.',
        security: [{ cookieAuth: [] }, { bearerAuth: [] }],
        parameters: [
          { name: 'subjectId', in: 'path', required: true, schema: { type: 'string' } },
          { name: 'bookId', in: 'path', required: true, schema: { type: 'string' } },
        ...
        requestBody: {
          required: true,
          content: { 'application/json': { schema: { type: 'object', properties: { ktpTopicIds: { type: 'array', items: { type: 'string' }, description: 'id тем КТП (KtpCatalog.topics._id)' } } } } }
        },
        responses: {
          200: { description: 'Маппинг сохранён; возвращён предмет' },
          400: { description: 'Темы КТП не найдены в справочнике' },
          404: { description: 'Предмет/книга/глава/тема не найдены' }
        }
      }
    },
    ...
    '/users/me/ent-progress': {
      get: {
        tags: ['Users'],
        summary: 'Прогноз балла ЕНТ (шкала 140) по накопленным результатам',
        security: [{ cookieAuth: [] }, { bearerAuth: [] }],
        responses: {
