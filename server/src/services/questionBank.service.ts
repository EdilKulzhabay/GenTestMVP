...
import { findTopic, topicAssetCandidates } from '../utils/roadmapChapter.util';
...
function buildTypeMix(count: number, offset: number): Partial<Record<EntQuestionType, number>> {
  ...
  for (let i = 0; i < count; i++) {
    const t =
      BANK_TYPE_CYCLE[
        (((offset + i) % BANK_TYPE_CYCLE.length) + BANK_TYPE_CYCLE.length) % BANK_TYPE_CYCLE.length
      ];
    mix[t] = (mix[t] ?? 0) + 1;
  }
  return mix;
...
function contentHashOf(q: IQuestion): string {
  ...
  const parts = [norm(q.questionText), ...(q.options ?? []).map(norm).sort(), `=${answerSig}`];
...
type ResolvedNode = {
  ...
  /** Resolved-сайдкар: union ассетов sourceRefs-тем (для теста узла). */
  assets: IContentAsset[];
  /** Кандидат-ассеты sourceRefs-тем (инъекция в промпт LLM-B банка). */
...
class QuestionBankService {
  ...
  private async resolveNode(subjectId: string, ktpTopicId: string): Promise<ResolvedNode> {
    ...
    const sources =
      (md.sources as Array<{ bookId: string; chapterId: string; topicId: string }> | undefined) ??
      [];
    ...
    const book = (
      subject as { books?: Array<{ _id?: { toString(): string }; contentLanguage?: string }> }
    ).books?.find(b => b._id?.toString() === primaryBookId);

    // Сайдкар + кандидат-ассеты sourceRefs-тем (дедуп по assetId).
    const assets: IContentAsset[] = [];
    const assetCandidates: IAssetCandidate[] = [];
    const assetSeen = new Set<string>();
    const candSeen = new Set<string>();
    for (const src of sources) {
      const topic = findTopic(subject, src.bookId, src.chapterId, src.topicId);
      if (!topic?.assets?.length) continue;
      for (const a of topic.assets) {
        const id = a._id ? String(a._id) : '';
        if (id && !assetSeen.has(id)) {
          assetSeen.add(id);
          assets.push(a);
        }
      }
      for (const cand of topicAssetCandidates(topic.title ?? '', topic.assets)) {
        if (!candSeen.has(cand.assetId)) {
          candSeen.add(cand.assetId);
          assetCandidates.push(cand);
        }
      }
    }
  ...
  async coverage(
    ...
    const active = await QuestionItem.find({
      subjectId,
  ...
  async generateForCoverage(
    ...
    created: number;
    rejected: number;
    coverage: Awaited<ReturnType<QuestionBankService['coverage']>>;
  }> {
    await this.assertSubject(subjectId);
    ...
    if (!resolved.sourceText.trim())
      throw AppError.badRequest('Node has no source text to ground questions');
        ...
        for (const bucket of buildDifficultyPlan(target.deficit, centerDifficulty)) {
          ...
          try {
            generated = await aiService.generateKcQuestions({
              ...
              ...(resolved.assetCandidates.length
                ? { assetCandidates: resolved.assetCandidates }
                : {}),
            });
          } catch (e) {
          ...
          for (const q of generated) {
            ...
            const verdict = await aiService.verifyQuestionItem({
              question: q,
              sourceText: resolved.sourceText,
            });
            if (!verdict.ok) {
  ...
  async assembleNodeTest(
    ...
    const resolved = await this.resolveNode(subjectId, ktpTopicId);
    if (!resolved.primary.bookId)
      throw AppError.badRequest('Node has no primary book to attach test');

    // Resolved-сайдкар узла (union ассетов sourceRefs-тем) — не поле схемы Test,
    // вешаем JS-свойством для sanitize (см. TestController.sanitize).
    const attachAssets = (t: InstanceType<typeof Test>): InstanceType<typeof Test> => {
      (t as unknown as { assets?: IContentAsset[] }).assets = resolved.assets;
      return t;
    };
    ...
    if (existing) return attachAssets(existing);
    ...
    const questions: IQuestion[] = selected.map(it => {
      ...
      return {
        ...
        ...(it.knowledgeComponentIds?.length
          ? { knowledgeComponentIds: it.knowledgeComponentIds }
          : {}),
      };
    });
