...
class AIService {
  ...
  async verifyQuestionItem(input: { question: IQuestion; sourceText: string }): Promise<{ ok: boolean; reason?: string }> {
    ...
    const prompt = [
      'Проверь тестовый вопрос на корректность ОТНОСИТЕЛЬНО текста источника.',
      'Верни строгий JSON: { "ok": true|false, "reason": "кратко" }.',
      'ok=true только если: (1) правильный ответ действительно верен, (2) он однозначно следует из источника,',
      '(3) среди вариантов нет второго столь же верного, (4) вопрос не опирается на факты вне источника.',
      '',
