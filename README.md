...
| Команда                     | Описание                                                                                        |
| --------------------------- | ----------------------------------------------------------------------------------------------- |
| `./deploy.sh`               | Тайпчек → пуш main → на сервере: pull, install, build, `pm2 restart`, ожидание health           |
| `SKIP_CHECKS=1 ./deploy.sh` | То же без локального тайпчека                                                                   |

Настройки по умолчанию (см. шапку `deploy.sh`): `ubuntu@46.247.40.26:/home/ubuntu/GenTestMVP`, pm2-процесс `server`, порт API 5111. SSH спросит пароль один раз за запуск.
